...
interface FilterBarProps {
  ...
  programs?: import("@/types/kdt").KDTProgram[];
}

export function FilterBar({ filters, onFilterChange, programs }: FilterBarProps) {
  const baseData = programs && programs.length > 0 ? programs : kdtPrograms;
  // 고유 값 추출
  const uniqueYears = [...new Set(baseData.map(p => p.년도))].sort((a, b) => b - a);
  const uniqueQuarters = [...new Set(baseData.map(p => p.분기))].sort((a, b) => {
    // 분기 순서: 1Q, 2Q, 3Q, 4Q
